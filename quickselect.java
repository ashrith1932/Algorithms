//using median of median 
//method similar to quicksort -> a pivot is chosen and its index checked with required index if pivot<index recurse to right
//else recurse to left
//can be applied at any pivot but to ensure O(n) time complexity median of median is used as pivot
...
public class quickselect {
    ...
    static int quickselectalgo(int[] arr,int left,int right,int k){
        if(left<=right){
            int  i = partition(arr,left,right,k);
            if(i==k-1){return arr[i];}//if the element at index is the required kth smallest
            else if(i<k-1){return quickselectalgo(arr,i+1,right,k);}//if the elemet at index is less than required kth smallest
            else{return quickselectalgo(arr,left,i-1,k);}//if the element at index is greater than required kth smallest
        }       
        return -1;
    }
    ...
    public static void main(String[] args) {
        ...
        int val = quickselectalgo(arr,0,arr.length-1,10);
